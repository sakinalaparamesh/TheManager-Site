...
class ControlleractionModel extends CI_Model {
    ...
    public function controlleractionUpdate($data, $id = '') {
        $res_data = 1;
        $dep_check = array(
            "actionid!=" => $id,
//            "controlleractionname" => $data["controlleractionname"],
//            "actioncodename"=>$data["actioncodename"]
        );
       // print_r($res_data); exit;
        $checkRes = $this->Model->check("tbl_mng_controlleractionmaster", $dep_check);
        ...
        if ($checkRes->num_rows() > 0) {
            ...
            if ($this->Model->update("tbl_mng_controlleractionmaster", $q_data, $data)) {
                $res_data = 1;
            } else {
                $res_data = 2;
            }
    ...
    public function delete_controlleraction($id) {
        $res_data = 1;
       ...
       // print_r($roles);exit;
        if ($roles->num_rows() == 0) {
            ...
            $this->Model->update("tbl_mng_controlleractionmaster", $q_data,array("isactive"=>0));
            $res_data = 1;
            }catch(Exception $e){
            $res_data = 3;
            }
