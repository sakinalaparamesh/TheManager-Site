...
class User_model extends CI_Model {
    ...
    public function userSave($data,$user_roles) {
        $res_data = 1;
        $user_check = array(
            "user_email" => $data["user_email"]
        );
        $checkRes = $this->Model->check("tdl_mng_usermaster", $user_check);
    ...
    public function getAllUsers($limit=10,$start=0,$search='',$order=null,$dir=null)
    { 
        try{
                $data = array();
                $data['totalFiltered'] = $this->getUserCountBySearch($search);
                //Filter records Data
                ...
                if($order == 1){
                    ...
                    $this->db->order_by('user_email', $dir);
                }
    ...
    public function getUserCountBySearch($search='')
    { 
        try{
                ...
                $this->db->from("tdl_mng_usermaster");
                if($search){
                    $this->db->like(array("CONCAT(user_name,' ',user_email)" => $search));
                }
                $query = $this->db->get();
